...
export const userQuacksQueryOptions = (username: string) =>
  queryOptions({
    queryKey: quackKeys.byUser(username),
    queryFn: async () => quacksSchema.parse(await api.get(`users/${username}/quacks`).json()),
  })
